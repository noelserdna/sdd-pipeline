...
8. For each requirement, create the corresponding specification:
   ...
   **Tier Classification:** For each invariant extracted, classify:
   ...
   After completing all UCs with their error flows and invariants, update `spec/DERIVED-SPECS.md`:

   1. For every Tier 2 artifact created (exception courses, invariants, BDD scenarios, API error codes), add an entry
   2. For every Tier 1 artifact, add an entry marked as **`[PENDING REQ]`**
