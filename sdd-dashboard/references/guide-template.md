...
<nav class="sidebar">
  ...
  <a href="#code-intelligence" class="sidebar-link">Code Intelligence</a>
...
<p>The SDD pipeline consists of <strong>20 skills</strong> (9 pipeline + 4 onboarding + 6 utility + 1 setup). Each skill reads the output of previous stages, applies engineering discipline, and produces formal artifacts.</p>
...
<h3>Context Augmentation Hook</h3>
<p>The <strong>H5 — Context Augmentation</strong> hook intercepts file operations (<code>Grep</code>, <code>Glob</code>, <code>Read</code>, <code>Edit</code>, <code>Write</code>) and injects SDD traceability context into tool results. When you read or search a file that is referenced in the traceability graph, the hook adds:</p>
<ul>
  <li><strong>Traceability chain</strong> — upstream requirements and downstream implementations</li>
  <li><strong>Coverage status</strong> — whether the artifact has tests, code, and specs linked</li>
  <li><strong>Last commit</strong> — most recent change to the artifact</li>
</ul>
<p>When <strong>code intelligence</strong> is available (via <code>/sdd-code-index</code>), the hook also injects:</p>
<ul>
  <li><strong>Callers &amp; callees</strong> — functions that call or are called by the current symbol</li>
  <li><strong>Processes</strong> — workflows or use cases that involve the symbol</li>
  <li><strong>Confidence scores</strong> — how reliably each reference was resolved</li>
</ul>


<h2 id="mcp-server">MCP Server</h2>

<p>SDD includes a <strong>Model Context Protocol (MCP) server</strong> that provides live traceability queries. The server reads <code>traceability-graph.json</code> (generated by <code>/sdd-dashboard</code>) and exposes it through a structured API.</p>

<div class="info-card">
  <h4>Prerequisite</h4>
  <p>Run <code>/sdd-dashboard</code> first to generate <code>traceability-graph.json</code>. The MCP server reads this file and watches for changes.</p>
</div>
...
<table class="metric-table">
  ...
  <tbody>
    <tr><td><strong>sdd_query</strong></td><td>Search artifacts by text, ID, or type. Returns matching nodes from the traceability graph.</td></tr>
    <tr><td><strong>sdd_impact</strong></td><td>Blast radius analysis — BFS traversal from a given artifact to find all affected nodes at a specified depth.</td></tr>
    <tr><td><strong>sdd_context</strong></td><td>360&deg; artifact view — shows all upstream and downstream connections for a single artifact.</td></tr>
    <tr><td><strong>sdd_coverage</strong></td><td>Gap analysis — identifies artifacts missing tests, code, or spec coverage, filterable by domain or layer.</td></tr>
    <tr><td><strong>sdd_trace</strong></td><td>Full traceability chain — follows the complete REQ&rarr;UC&rarr;WF&rarr;API&rarr;BDD&rarr;INV&rarr;ADR path for an artifact.</td></tr>
  </tbody>
</table>

<h3>Resources (7)</h3>
<p>The server exposes <code>sdd://</code> URIs for direct artifact access:</p>
<ul>
  <li><code>sdd://graph</code> — full traceability graph</li>
  <li><code>sdd://pipeline-status</code> — current pipeline state</li>
  <li><code>sdd://artifact/{id}</code> — specific artifact by ID</li>
  <li><code>sdd://coverage</code> — coverage summary</li>
  <li><code>sdd://health</code> — health score breakdown</li>
  <li><code>sdd://stale</code> — list of stale artifacts</li>
  <li><code>sdd://hints</code> — next-step suggestions (GitNexus pattern)</li>
</ul>

<h3>Prompts (2)</h3>
<ul>
  <li><strong>analyze_impact</strong> — generates a structured impact report for a proposed change</li>
  <li><strong>generate_status_report</strong> — produces a pipeline status summary for stakeholders</li>
</ul>


<h2 id="code-intelligence">Code Intelligence</h2>

<p>The <strong>Code Intelligence</strong> system bridges SDD traceability with deep code analysis. It maps symbols (functions, classes, modules) to SDD artifacts, enabling questions like "which requirements does this function implement?" and "what code is affected by changing this use case?"</p>

<h3>How It Works</h3>
<ol>
  <li>Run <code>/sdd-code-index</code> to analyze the codebase and generate a symbol index</li>
  <li>The index maps each symbol to its callers, callees, and SDD artifact references</li>
  <li>The context augmentation hook (H5) uses this index to enrich file reads with traceability</li>
  <li>The MCP server integrates code intelligence data into blast radius and coverage queries</li>
</ol>
...
<table class="metric-table">
  ...
  <tbody>
    <tr><td><strong>Full</strong></td><td>Complete codebase analysis — symbols, call graph, SDD mappings</td></tr>
    <tr><td><strong>Lite</strong></td><td>Quick scan — file-level references only, no call graph</td></tr>
    <tr><td><strong>Status</strong></td><td>Show current index state without re-analyzing</td></tr>
    <tr><td><strong>Refresh</strong></td><td>Incremental update — only re-analyze changed files</td></tr>
  </tbody>
</table>

<div class="info-card">
  <h4>Optional: GitNexus</h4>
...
<table class="metric-table">
  ...
  <tbody>
    ...
    <tr><td><strong>Code Index</strong></td><td><code>/sdd-code-index</code></td><td>Indexes project code for deep traceability; bridges GitNexus code intelligence with SDD artifacts</td></tr>
...
<div class="glossary-grid">
  ...
  <div class="glossary-item"><div class="glossary-abbr">MCP Server</div><div class="glossary-def">Model Context Protocol server providing live traceability queries via sdd:// URIs</div></div>
  <div class="glossary-item"><div class="glossary-abbr">Code Intelligence</div><div class="glossary-def">System that maps code symbols to SDD artifacts for deep traceability</div></div>
  <div class="glossary-item"><div class="glossary-abbr">GitNexus</div><div class="glossary-def">Optional external tool for advanced call graph analysis and cross-repo references</div></div>
  <div class="glossary-item"><div class="glossary-abbr">Symbol</div><div class="glossary-def">A code element (function, class, module) tracked by code intelligence</div></div>
  <div class="glossary-item"><div class="glossary-abbr">Confidence Score</div><div class="glossary-def">0-1 value indicating reliability of an inferred code-to-artifact reference</div></div>
  <div class="glossary-item"><div class="glossary-abbr">Inferred Reference</div><div class="glossary-def">A code-to-artifact link detected heuristically (naming, comments, proximity)</div></div>
