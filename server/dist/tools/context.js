...
export const CONTEXT_TOOL = {
    name: "sdd_context",
    description: "Get a 360-degree view of an artifact: its definition, all upstream/downstream connections, code references, test references, commit references, and coverage gaps. Essential before modifying any artifact.",
    inputSchema: {
        type: "object",
        properties: {
            artifact_id: {
...
export function executeContext(args, graph, index) {
    ...
    if (artifact.type === "REQ") {
        ...
        if (!hasUC)
            gaps.push("MISSING_UC: No use case implements this requirement");
        if (!hasBDD)
            gaps.push("MISSING_BDD: No BDD scenario verifies this requirement");
        if (!hasTask)
            gaps.push("MISSING_TASK: No task decomposes this requirement");
    ...
    if (graph.codeIntelligence?.indexed) {
        ...
        const symbols = ci.symbols.filter((s) => s.artifactRefs.includes(artifact_id) ||
            s.inferredRefs.includes(artifact_id));
        if (symbols.length > 0) {
            const processes = ci.processes.filter((p) => p.artifactRefs.includes(artifact_id));
            codeIntel = {
