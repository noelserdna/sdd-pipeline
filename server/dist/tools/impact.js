...
export const IMPACT_TOOL = {
    name: "sdd_impact",
    description: "Analyze blast radius of changing an artifact. Uses BFS by depth to classify impact: depth 1 = WILL_BREAK, depth 2 = LIKELY_AFFECTED, depth 3 = MAY_NEED_REVIEW. When code intelligence is available, uses call graph for deeper analysis.",
    inputSchema: {
        type: "object",
        properties: {
            artifact_id: {
...
export function executeImpact(args, graph, index) {
    ...
    for (const items of Object.values(byDepth)) {
        for (const item of items) {
            ...
            if (art?.stage)
                affectedStages.add(art.stage);
        }
    }
    // Risk assessment
    const totalAffected = Object.values(byDepth).reduce((sum, arr) => sum + arr.length, 0);
    let risk;
    if ((byDepth[1]?.length ?? 0) > 5 || totalAffected > 20)
        risk = "HIGH";
    else if ((byDepth[1]?.length ?? 0) > 2 || totalAffected > 10)
        risk = "MEDIUM";
    else
        risk = "LOW";
    ...
    if (graph.codeIntelligence?.indexed) {
        ...
        const relatedSymbols = ci.symbols.filter((s) => s.artifactRefs.includes(artifact_id) ||
    ...
    const output = {
        ...
        risk,
        totalAffected,
        byDepth: Object.fromEntries(Object.entries(byDepth).map(([d, items]) => [
            d,
            {
