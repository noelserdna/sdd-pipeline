...
export function createSDDServer() {
    ...
    // Tools
    // -----------------------------------------------------------------------
    server.tool("sdd_query", "Search SDD artifacts by text, ID, type, or domain. Returns matching artifacts with relevance scores.", {
    ...
    server.tool("sdd_impact", "Analyze blast radius of changing an artifact. BFS by depth: d1=WILL_BREAK, d2=LIKELY_AFFECTED, d3=MAY_NEED_REVIEW.", {
    ...
    server.tool("sdd_context", "360-degree view of an artifact: definition, upstream/downstream, code/test/commit refs, and coverage gaps.", {
    ...
    server.tool("sdd_coverage", "Analyze traceability coverage gaps by business domain and technical layer.", {
        domain: z.string().optional().describe("Filter by business domain"),
    ...
    server.tool("sdd_trace", "Trace the complete chain: REQ->UC->WF->API->BDD->INV->ADR->TASK->COMMIT->CODE->TEST. Reports breaks and status.", {
    ...
    // Prompts
    // -----------------------------------------------------------------------
    server.prompt("analyze_impact", "Pre-change workflow: context, blast radius, chain integrity, and cascade recommendation.", { artifact_id: z.string().describe("The artifact ID to analyze before changing") }, async (args) => {
        return getPrompt("analyze_impact", args);
    });
    server.prompt("generate_status_report", "Pipeline health report: state, coverage, priority actions, and assessment.", async () => {
